...
export enum GoogleAuthErrors{
    ...
    INTERNAL_ERROR = 'InternalError', /* note: fixed google's misspelling here, it's INTNERNAL_ERROR in the app */
...
export class GoogleAuthError extends OAuth.OAuthError{
	static from(error: string): GenericError{
		switch(error){
			...
			case GoogleAuthErrors.BAD_TOKEN_REQUEST:
			...
			case GoogleAuthErrors.INTERNAL_ERROR:
		...
		return new GoogleAuthError(error);
	}
}